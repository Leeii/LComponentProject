 ...
 * Progress Wheel https://github.com/Todd-Davies/ProgressWheel
 *
...
public class ProgressWheel extends View {
    ...
    private void setupBounds(int layout_width, int layout_height) {
        ...
        if (!fillRadius) {
            ...
            // circle
            int minValue = Math.min(layout_width - paddingLeft
                            - paddingRight,
                    layout_height - paddingBottom - paddingTop);

            int circleDiameter = Math.min(minValue,
                    circleRadius * 2 - barWidth * 2);

            // Calc the Offset if needed for centering the wheel in the
            // available space
            int xOffset = (layout_width - paddingLeft
                    - paddingRight
                    - circleDiameter)
                    / 2 + paddingLeft;
            int yOffset = (layout_height - paddingTop
                    - paddingBottom
                    - circleDiameter)
                    / 2 + paddingTop;
    ...
    private void parseAttributes(TypedArray a) {
        ...
        circleRadius =
                (int) a.getDimension(R.styleable.ProgressWheel_circleRadius,
                        circleRadius);
        ...
        if (a.getBoolean(R.styleable.ProgressWheel_progressIndeterminate, true)) {
            spin();
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isSpinning) {
            ...
            if (mProgress != mTargetProgress) {
                ...
                float deltaTime = (float) (SystemClock.uptimeMillis()
                        - lastTimeAnimated)
                        / 1000;
